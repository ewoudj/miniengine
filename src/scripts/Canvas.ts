...
export class CanvasRenderer {
    ...
    private resize(): void {
		...
		this.offsetTop = Math.ceil((canvas.height - (this.engine.height * this.scale)) / 2);
	...
	private renderEntity (entity: IEntity): void {
		...
		if(entity.model) {
			this.drawRects({x: entity.position.x, y: entity.position.y}, entity.model, entity.color, true);
		}
		if(entity.texts){
			for(const t of entity.texts ){
				this.context.font = Math.ceil((t.size || 50) * this.scale) + 'px ' + t.font;
				this.context.textAlign = t.alignment;
				this.context.fillStyle = t.color;
				if(t.alignment === 'center'){
					this.context.fillText(t.text, 
	...
	private drawRects (offset: IPoint, rects: IRectangle[], color: string, fill: boolean): void{
        rects.forEach( r => this.drawRect(offset, r, color, fill));
	}
	
	private drawRect (offset: IPoint, rect: IRectangle, color: string, fill: boolean): void{
		if(fill){
