    ...
    public update (time: number){
        ...
        for(const entity of this.engine.entities){
            if(entity instanceof Ship){
                const lightnessRed = Math.floor( 128 + ((127 * (Math.sin( (time + (i * 300)) / 800)))));
                const lightnessGreen = Math.floor( 128 + ((127 * (Math.cos( (time + (i * 300)) / 800)))));
                const lightnessBlue = Math.floor( 128 + ((127 * (Math.cos( (time + (i * 250)) / 400)))));
