...
export class Ship extends GameEntity { 
	...
	public update (time: number){
		...
		if(this.collisions){
			for(const collider of this.collisions){
				...
				const colliderIsSpawnStarWhileInvulnerable = (this.invulerability && collider === this.spawnStar);
				if(owner !== this && !colliderIsSpawnStarWhileInvulnerable){
					if(collider instanceof Star && collider.parent && collider.color !== this.color){
	...
	private updateControllerState (){
		...
		if(this.type !== 'computer'){
			...
			this.mousePosition = this.correctMousePosition( this.engine.controller.mousePosition, this.engine.controller.absoluteController );
		}
		else if(this.type === 'computer'){
			// In the case of AI the active AI will set the controller 
			const controls = this.name === 'Player 1' ? heuristic(this) : heuristic(this);
	...
	private calculateMovement (currentPosition: IPoint, mousePosition: IPoint, speedLimit: number, timeDelta: number){
