...
export function rectInRect (offsetA: IPoint, rectA: IRectangle, offsetB: IPoint, rectB: IRectangle): boolean{
	if(offsetA && rectA && offsetB && rectB){
		...
		const br = {x: rectA.x + rectA.w + offsetA.x, y: rectA.y + rectA.h + offsetA.y};
		return (
...
export function pointInRect (point: IPoint, offset: IPoint, rect: IRectangle): boolean{
...
export function lineIntersect(a: IPoint[],b: IPoint[]): IPoint | undefined {
	return checkIntersection(a[0].x, a[0].y, a[1].x, a[1].y, b[0].x, b[0].y, b[1].x, b[1].y);
}

function checkIntersection(x1: number, y1: number, x2: number, y2: number, x3: number, y3: number, x4: number, y4: number): IPoint | undefined  {
...
export function inflateRectangle(rectangle: IRectangle, amount: number): IRectangle{
	return {
...
export function moveRectangle(rectangle: IRectangle, direction: IPoint): IRectangle{
	return {
		h: rectangle.h,
		w: rectangle.w,
