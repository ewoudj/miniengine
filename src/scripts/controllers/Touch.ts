...
interface IWhatWGEventListenerArgs {
	capture?: boolean;
}
  
interface IWhatWGAddEventListenerArgs extends IWhatWGEventListenerArgs {
	passive?: boolean;
	once?: boolean;
}
...
type WhatWGAddEventListener = (
	...
	options?: IWhatWGAddEventListenerArgs
) => void;
...
export class TouchController {
	...
	constructor(engine: Engine){
		...
		if(this.engine.touchable) {
			...
			(window.addEventListener as WhatWGAddEventListener)( 'touchmove', this.onTouchMove.bind(this), {passive : false} );
	...
	private drawLine (c: CanvasRenderingContext2D, color: string, lineWidth: number, start: IPoint, end: IPoint) { // c is the canvas' context 2D
		c.beginPath(); 
		c.strokeStyle = color; 
	...
	private drawCircle(c: CanvasRenderingContext2D, color: string, lineWidth: number, center: IPoint, radius: number) { // c is the canvas' context 2D
		c.beginPath(); 
		c.strokeStyle = color; 
		c.lineWidth = lineWidth; 
