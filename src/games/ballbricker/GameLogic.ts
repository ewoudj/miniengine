...
import { ILogic, Logic } from './Logic';
...
export class GameLogic implements ILogic {
  ...
  public update(time: number): void {
    ...
    if (
      pointInRect(
        ...
        this.bat.position,
        inflateRectangle(
          { h: this.bat.height, w: this.bat.width, x: 0, y: 0 },
          this.ball.radius
        )
      )
    ) {
  ...
  private createDefaultBallAndBat() {
    // Create the ball
    this.ball = this.engine.add(
      new Ball(this.engine, {
        color: '#FFF',
        position: {
    ...
    // Create the bat
    this.bat = this.engine.add(
      new Brick(this.engine, {
        kind: 'Bat',
        color: '#FFF',
        height: 18,
        indestructable: true,
