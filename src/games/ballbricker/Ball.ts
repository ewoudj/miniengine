...
export class Ball extends EntityBase implements IDrawable {
  ...
  public update(time: number) {
    ...
    while (previousPosition !== undefined) {
      const nearestIntersection = this.getNearestIntersection(
        ballMovement,
        previousPosition
      );
  ...
  public drawShadow(context: CanvasRenderingContext2D): void {
    ...
    context.beginPath();
    context.arc(
      (this.position.x + shadowOffsetLeft) * s +
        this.engine.renderer.offsetLeft,
      (this.position.y + shadowOffsetTop) * s + this.engine.renderer.offsetTop,
      this.radius * s,
      0,
      2 * Math.PI,
      false
  ...
  public draw(context: CanvasRenderingContext2D): void {
    ...
    context.beginPath();
    context.arc(
      this.position.x * s + this.engine.renderer.offsetLeft,
      this.position.y * s + this.engine.renderer.offsetTop,
      this.radius * s,
      0,
      2 * Math.PI,
      false
    ...
    context.beginPath();
    context.arc(
      (this.position.x - this.radius / 4) * s + this.engine.renderer.offsetLeft,
      (this.position.y - this.radius / 4) * s + this.engine.renderer.offsetTop,
      this.radius * 0.6 * s,
      0,
      2 * Math.PI,
      false
  ...
  private getCollisionRect(brick: Brick): IRectangle {
    return inflateRectangle(
      moveRectangle(
        { h: brick.height, w: brick.width, x: 0, y: 0 },
        brick.position
      ),
      this.radius
    );
  ...
  private getNearestIntersection(
    ballMovement: IPoint[],
  ...
  private getIntersections(ballMovement: IPoint[]) {
    ...
    for (const brick of this.engine.entities) {
      if (brick instanceof Brick) {
        ...
        this.addIntersection('TOP', brick, intersections, ballMovement, [
          { x: r.x, y: r.y },
          { x: r.x + r.w, y: r.y },
        ]);
        this.addIntersection('BOTTOM', brick, intersections, ballMovement, [
          { x: r.x, y: r.y + r.h },
          { x: r.x + r.w, y: r.y + r.h },
        ]);
        this.addIntersection('LEFT', brick, intersections, ballMovement, [
          { x: r.x, y: r.y },
          { x: r.x, y: r.y + r.h },
        ]);
        this.addIntersection('RIGHT', brick, intersections, ballMovement, [
          { x: r.x + r.w, y: r.y },
          { x: r.x + r.w, y: r.y + r.h },
        ]);
      }
