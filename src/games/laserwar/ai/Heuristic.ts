...
export function heuristic(gameEntity: GameEntity): IControllerState {
  ...
  for (const e of gameEntity.engine.entities) {
    const ownedLaserBeam =
      e instanceof LaserBeam ? e.owner === gameEntity : false;
    if (e !== gameEntity && !ownedLaserBeam && e.position) {
      ...
      if (
        e instanceof Star &&
        e.colorIndex !== gameEntity.colorIndex &&
        d < nearestStarDistance
      ) {
        nearestStarDistance = d;
        nearestStar = e;
      }
      if (
        e instanceof GameEntity &&
        (e.type === 'player' || e.type === 'computer')
      ) {
        otherShip = e;
      }
    }
  ...
  if (
    (gameEntity.position && nearestEntityDistance < 50) ||
    gameEntity.evadingTime > 0
  ) {
    if (nearestEntity != null) {
      ...
      gameEntity.mousePosition = rotate(
        gameEntity.position,
    ...
    if (!evading) {
      ...
      gameEntity.mousePosition = {
        x:
          ...
          gameEntity.targetVector.x * (movex ? 100 : reversex ? -100 : 0),
        y:
          gameEntity.position.y + gameEntity.targetVector.y * (movey ? 100 : 0),
      };
    }
    if (
      gameEntity.position.x === previousPosition.x &&
